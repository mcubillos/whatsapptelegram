...
class MediaUploader(WARequest, threading.Thread):
    ...
    def encryptImg(self, img):
        refkey = binascii.hexlify(os.urandom(112))
        img = self.pad(img)
        derivative = HKDFv3().deriveSecrets(binascii.unhexlify(refkey), binascii.unhexlify("57686174734170702049d616765204b657973"),112)
        parts = ByteUtil.split(derivative,16,32)
        iv = parts[0]
        cipherKey = parts[1]
        cipher = AES.new(key=cipherKey, mode=AES.MODE_CBC, IV=iv)
        imgEnc = cipher.encryt(img)
        return img[:-6]
    ...
    def run(self):
        ...
        try:
            ...
            stream1 = f.read()
            stream = self.encryptImg(stream1)
            f.close()
            status = 0
            lastEmit = 0
